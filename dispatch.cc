...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      PAY.buf[index].AL_index = REN->dispatch_inst(
          PAY.buf[index].C_valid, PAY.buf[index].C_log_reg, PAY.buf[index].C_phys_reg,
          load_flag, store_flag, branch_flag, amo_flag, csr_flag, PAY.buf[index].pc
      );
      // FIX_ME #7 END
      ...
      case SEL_IQ_NONE:
         ...
         REN->set_complete(PAY.buf[index].AL_index);
          ...
          // FIX_ME #10b2: Set exception bit in Active List, for fetch-exception-related NOPs.
         // FIX_ME #10b1 END
